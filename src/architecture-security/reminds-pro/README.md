...
We understand that our customers need to be confident using Reminds Pro, and be aware of our data collection practices.
...
Through the implementation of its different features, Reminds Pro access, processes and stores several kind of data:
- Reminders
- Users data
...
| Reminders | ✔ | 🚫 | ✔ | Accessed and updated of using the calendar.<br/>Stored as long as the organization is active. |
...
|```offline_access```| Gives the app access to resources on behalf of the user for an extended time. | **When a user approves the offline_access scope, Reminds Pro can receive refresh tokens from the Microsoft identity platform token endpoint. Refresh tokens are long-lived. Reminds Pro can get new access tokens as older ones expire.** | No |
